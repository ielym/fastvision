...
def model_fn(args, device):
    ...
    if args.pretrained_weights:
        model = LoadStatedict(model=model, weights=args.pretrained_weights, device=device, strict=True)
...
def Inference(args, device):
    ...
    for file_name in file_names:
        ...
        ret, frame = cap.read()
        while ret:
            cv2.imshow(f'{category_id_name_map[category]}', frame)
            cv2.waitKey(10)
            ret, frame = cap.read()
