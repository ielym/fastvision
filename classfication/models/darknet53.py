...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, mid_channels):
        ...
        self.shortcut = (in_channels == mid_channels * 2)
...
class Darknet(nn.Module):
    ...
    def forward(self, x):
        ...
        conv0 = self.conv0(x) # torch.Size([1, 32, 416, 416])

        conv1 = self.conv1(conv0) # torch.Size([1, 64, 208, 208])
        res1 = self.res1(conv1) # torch.Size([1, 64, 208, 208])

        conv2 = self.conv2(res1) # torch.Size([1, 128, 104, 104])
        res2 = self.res2(conv2) # torch.Size([1, 128, 104, 104])

        conv3 = self.conv3(res2) # torch.Size([1, 256, 52, 52])
        res3 = self.res3(conv3) # torch.Size([1, 256, 52, 52])

        conv4 = self.conv4(res3) # torch.Size([1, 512, 26, 26])
        res4 = self.res4(conv4) # torch.Size([1, 512, 26, 26])

        conv5 = self.conv5(res4) # torch.Size([1, 1024, 13, 13])
        res5 = self.res5(conv5) # torch.Size([1, 1024, 13, 13])
