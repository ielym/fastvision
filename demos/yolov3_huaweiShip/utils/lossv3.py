...
from .iou import wh_iou_batch, xywh_iou_batch, CIOU
...
class ComputeLoss(nn.Module):
    ...
    def forward(self, predict_layers, target_all, model):
        ...
        loss_box = torch.zeros(1).to(predict_layers[0])
        ...
        for layer_idx in range(len(predict_layers)):
            ...
            target_offset_xy = target[:, 2:4] - target_grid_xy.float()

            # ==================================== summary target ====================================
            # bs_idx, cls_idx, x, y, w, h, grid_x, grid_y, offset_x, offset_y, matched_anchor_idx, matched_anchor_w, matched_anchor_h
            target = torch.cat([target, target_grid_xy, target_offset_xy, max_iou_idx_targets_and_anchors.unsqueeze(1), anchors_corresponding_to_targets], dim=1).float() # torch.Size([10, 14]) feature-scale
            ...
            predict_xywh = torch.cat([predict_xy.float()+grid_xy.float(), predict_wh.float()], dim=4) # torch.Size([2, 14, 14, 3, 4]) feature-scale

            # weight bbox
            weight_bbox = 2 - ((target[:, 4] * target[:, 5] / (feature_height * feature_width)) ** 2).view(-1, 1)
            ...
            # loss_xy += F.mse_loss(predict_xy_for_loss.sigmoid(), target_xy_for_loss)

            # ==================================== loss wh ====================================+-
            # predict_wh_for_loss = predict[target[:, 0].long(), target[:, 7].long(), target[:, 6].long(), target[:, 10].long(), 2:4] # torch.Size([10, 2])
            # target_wh_for_loss = torch.log((target[:, 4:6].float() / target[:, 11:13].float()) + 1e-14) # TODO : changed
            # loss_wh += F.mse_loss(predict_wh_for_loss, target_wh_for_loss)

            # ==================================== IOU Loss ====================================+-
            predict_xywh_for_loss = predict_xywh[target[:, 0].long(), target[:, 7].long(), target[:, 6].long(), target[:, 10].long(), :]
            target_xywh_for_loss = target[:, 2:6]
            ciou_targets_and_predicts = CIOU(predict_xywh_for_loss, target_xywh_for_loss, mode='xywh', eps=1e-7)
            loss_box += (1 - ciou_targets_and_predicts).mean()
            ...
            # loss_cls += F.binary_cross_entropy(predect_cls_for_loss.sigmoid(), target_cls_for_loss)
            # loss_cls += self.CrossEntropyLoss(predect_cls_for_loss, target[:, 1].long())
            ...
            # loss_conf += F.binary_cross_entropy(predict_conf_for_loss.sigmoid(), target_conf_for_loss)

        # print(loss_box.item(), loss_cls.item(), loss_conf.item())

        return loss_box, loss_cls, loss_conf